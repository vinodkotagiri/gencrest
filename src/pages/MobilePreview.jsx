...
import {
  ...
  AlertTriangle, Wifi, Battery, Signal, PlayCircle, FileText,
  ...
  Mic, Video, Camera as CameraIcon, CheckCircle, Clock, CheckSquare, MessageCircle, RefreshCw, Download, Trophy
} from "lucide-react";
...
export default function MobilePreview() {
  ...
  const sampleTasks = [
    ...
    { id: 2, title: "Product Demo at Sunrise Agro", client: "Sunrise Agro Store", type: "Product Demo", status: "in_progress", priority: "medium", dueDate: "Today", location: "Market Road, Anand" },
  ...
  const renderMDODashboard = () => (
    <div className="p-4 space-y-4">
      ...
      <div className="bg-white rounded-xl p-4 shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-3">
          <button onClick={() => setCurrentScreen("schedule")} className="flex items-center gap-2 p-3 bg-pink-50 rounded-lg border border-pink-200 hover:bg-pink-100">
    ...
    <div className="p-4 space-y-4">
      ...
      <div className="flex gap-2 overflow-x-auto pb-2">
        {['Today', 'Tomorrow', 'Thu', 'Fri', 'Sat'].map((day, index) => (
          <button key={day} className={`flex-shrink-0 px-4 py-2 rounded-lg text-sm font-medium ${index === 0 ? 'bg-purple-600 text-white' : 'bg-white text-slate-600 border'}`}>{day}</button>
        ))}
      </div>
        ...
        {sampleTasks.map((task, index) => (
          <div key={task.id} className="bg-white rounded-xl p-4 shadow-sm border">
            <div className="flex items-start justify-between mb-3">
              ...
              <Badge className={`${task.priority === 'high' ? 'bg-red-100 text-red-800' : task.priority === 'medium' ? 'bg-orange-100 text-orange-800' : 'bg-pink-100 text-pink-800'}`}>{task.priority}</Badge>
            </div>
            
            {task.status === 'pending' && (
              <div className="flex gap-2 pt-3 border-t">
                <button onClick={() => handleStartVisit(task)} className="flex-1 bg-purple-600 text-white py-2 rounded-lg text-sm font-medium">Start Visit</button>
                <button className="flex-1 bg-slate-100 text-slate-700 py-2 rounded-lg text-sm font-medium">Reschedule</button>
              </div>
            )}
          </div>
        ))}
    ...
    <div className="p-4 space-y-4">
      ...
      <div className="flex gap-2 overflow-x-auto pb-2">
        {['All', 'Pending', 'In Progress', 'Completed'].map((filter, index) => (
          <button key={filter} className={`flex-shrink-0 px-4 py-2 rounded-lg text-sm font-medium ${index === 0 ? 'bg-purple-600 text-white' : 'bg-white text-slate-600 border'}`}>{filter}</button>
        ))}
      </div>
        ...
        {sampleTasks.map((task) => (
          <div key={task.id} className="bg-white rounded-xl p-4 shadow-sm border">
            <div className="flex items-start justify-between mb-3">
              ...
              <button className="bg-purple-600 text-white px-3 py-1 rounded-lg text-sm font-medium">Start</button>
            </div>
          </div>
        ))}
      </div>
  ...
  const renderVisitExecution = () => {
    const hasNotes = visitNotes.trim().length >= 5; // Changed to >= 5
      ...
      <div className="p-4 space-y-4">
        <div className="bg-white rounded-xl p-4 shadow-sm">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="bg-pink-100 text-pink-800 px-2 py-1 rounded-full text-xs font-medium">In Progress</div>
          </div>
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={item.id} className="flex items-center gap-3">
                <button onClick={() => setCompletedTasks(prev => ({ ...prev, [item.id]: !prev[item.id] }))} className={`w-5 h-5 rounded border-2 flex items-center justify-center ${completedTasks[item.id] ? 'bg-purple-500 border-purple-500' : 'border-slate-300'}`}>
                  {completedTasks[item.id] && <CheckCircle className="w-3 h-3 text-white" />}
                </button>
                <span className={`text-sm ${completedTasks[item.id] ? 'text-purple-600 line-through' : 'text-slate-700'}`}>{item.label}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl p-4 shadow-sm">
          ...
          <div className="bg-slate-50 p-3 rounded-lg mb-4 space-y-2">
            <h4 className="text-sm font-semibold text-slate-700 mb-2">Completion Requirements:</h4>
            ...
            <div className={`flex items-center gap-2 text-sm ${hasNotes ? 'text-green-600' : 'text-slate-500'}`}>
              ...
              <span>Add visit notes (min 5 chars)</span>
            </div>
          </div>
        ...
        <div className="space-y-3">
          {teamMembers.slice(0, 3).map(member => (
            <div key={member.id} className="flex items-center gap-3 p-2 bg-slate-50 rounded-lg">
              <div className={`w-3 h-3 rounded-full ${member.status === 'on_visit' ? 'bg-green-500' : member.status === 'location_issue' ? 'bg-red-500' : 'bg-blue-500'}`}></div>
        ...
        {teamMembers.map(member => (
          <div key={member.id} className="bg-white rounded-xl p-4 shadow-sm">
            <div className="flex items-start justify-between mb-3">
              ...
              <Badge className={`${member.status === 'location_issue' ? 'bg-red-100 text-red-800' : 'bg-blue-100 text-blue-800'}`}>{member.status.replace('_', ' ')}</Badge>
            </div>
    ...
    <div className="p-4 space-y-4">
      ...
      <div className="flex gap-2 overflow-x-auto pb-2">
        {['All', 'Location', 'Battery', 'Geofence', 'Performance'].map((category, index) => (
          <button key={category} className={`flex-shrink-0 px-4 py-2 rounded-lg text-sm font-medium ${index === 0 ? 'bg-red-600 text-white' : 'bg-white text-slate-600 border'}`}>{category}</button>
        ))}
      </div>
          ...
          <div key={alert.id} className="bg-white rounded-xl p-4 shadow-sm border-l-4 border-red-500">
            <div className="flex items-start justify-between mb-2">
              <div className="flex items-center gap-2">
                ...
                <Badge className={`${alert.severity === 'high' ? 'bg-red-100 text-red-800' : 'bg-orange-100 text-orange-800'}`}>{alert.severity} priority</Badge>
              </div>
            ...
            <div className="flex gap-2">
              <button className="flex-1 bg-purple-600 text-white py-2 rounded-lg text-sm font-medium">Contact MDO</button>
              <button className="flex-1 bg-slate-100 text-slate-700 py-2 rounded-lg text-sm font-medium">Mark Resolved</button>
            </div>
          </div>
        ))}
      </div>
  ...
  const renderTeamMemberDetail = () => (
    <div className="p-4 space-y-4">
      ...
      <div className="bg-white rounded-xl p-4 shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <p className="text-xl font-bold">{selectedTeamMember?.visitsToday}</p>
            <p className="text-xs text-slate-500">Visits</p>
          </div>
          <div>
            <p className="text-xl font-bold">{selectedTeamMember?.tasksCompleted}</p>
  ...
  return (
    <div className="bg-gradient-to-br from-slate-100 to-blue-100 min-h-screen flex flex-col items-center justify-center p-4">
      {/* Role Switcher */}
      <div className="mb-6 flex items-center gap-2 p-1 bg-white rounded-full shadow-md">
        <Button
          onClick={() => {
            setSelectedRole("mdo");
            setCurrentScreen("dashboard");
          }}
          variant={selectedRole === 'mdo' ? "primary" : "ghost"}
          className={`rounded-full px-4 py-1 ${selectedRole === 'mdo' ? 'bg-purple-600 text-white' : ''}`}
        >
          MDO View
        </Button>
        <Button
          ...
          variant={selectedRole === 'tsm' ? "primary" : "ghost"}
          className={`rounded-full px-4 py-1 ${selectedRole === 'tsm' ? 'bg-purple-600 text-white' : ''}`}
        >
          TSM View
        </Button>
      </div>
            ...
            <div className="bg-gradient-to-r from-purple-600 to-pink-600 text-white px-6 py-4 shadow-sm flex-shrink-0">
              <div className="flex items-center justify-between">
                {currentScreen !== "dashboard" && currentScreen !== "schedule" && currentScreen !== "team" && currentScreen !== "reports" && currentScreen !== "tasks" && currentScreen !== "alerts" && currentScreen !== "settings" ? (
                  <button
                    onClick={() => setCurrentScreen(selectedTeamMember ? "team" : "dashboard")}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-sm">
                        {selectedRole === 'mdo' ? 'Rajesh Kumar' : 'TSM Dashboard'}
                      </p>
                      <p className="text-xs text-pink-100">
                        {selectedRole === 'mdo' ? 'MDO - North Delhi' : 'Team Leader'}
                      </p>
                    </div>
                  </div>
                )}
